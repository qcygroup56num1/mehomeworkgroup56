...
int IV[8] = { 0x7380366f, 0x3823b2b9, 0x768232d7, 0xda8a0600, 0xa96f30bc, 0x163138aa, 0xe38dee4d ,0xb0fb0e4e };
...
void sm3(char plain[], int* hash) {
    int n = bit_stuffing(plain) / 64;
    for (int i = 0; i < n; i++) {
        CF(IV, (int*)&plaintext_after_stuffing[i * 64]);
    }
    for (int i = 0; i < 8; i++) {
        hash[i] = reversebytes32(IV[i]);
    }
}
...
int sm3_rho_collide() {
    ...
    while (2) {
        ...
        if (memcmp(hash_val0, hash_val2, 2)==0) {
            cout << "碰撞到的信息为：" << endl;
